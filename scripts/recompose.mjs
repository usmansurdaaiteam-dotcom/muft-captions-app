...
const prompt = makeV2CompositionPrompt(tokens, { language: { label: 'English + Urdu', romanize: true } });
const budgetLine = prompt.split('\n').find(l => l.includes('Mark AT MOST'));
console.log(`  prompt says: ${budgetLine ? budgetLine.trim() : '(no budget line found)'}\n`);

console.log('  composing...');
const { text, via } = await compose(prompt);
const parsed = parseV2CompositionResponse(text, tokens);
if (!parsed || !parsed.compositions || !parsed.compositions.length) {
  console.error('  Composer returned nothing usable.');
  process.exit(1);
}
...
console.log(`  via ${via}: ${parsed.compositions.length} lines returned\n`);
